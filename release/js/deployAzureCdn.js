...
function blobExists(blobService, options, destFileName){
    var deferred = Q.defer();
    blobService.getBlobMetadata(options.containerName, destFileName, function(error){
...
module.exports = function deploy(opt, files, loggerCallback, cb) {
    ...
    async.eachLimit(files, options.concurrentUploadThreads, function (file, eachCallback) {
        ...
        var check = options.override ? Q.resolve(false) : blobExists(blobService, options, destFileName);
