...
if($CreateSelfSignedCertificate)
{
    ...
    if($PSPKIVersion.Major -ieq 3 -And $PspkiVersion.Minor -ieq 2 -And $PspkiVersion.Build -ieq 5) {
        New-SelfsignedCertificateEx -Subject "CN=$DnsName" -EKU "Server Authentication", "Client authentication" -KeyUsage "KeyEncipherment, DigitalSignature" -Path $NewPfxFilePath -Password $securePassword -Exportable -NotAfter (Get-Date).AddYears(50)
    }
    else {
        New-SelfSignedCertificate -CertStoreLocation Cert:\CurrentUser\My -DnsName $DnsName -Provider 'Microsoft Enhanced Cryptographic Provider v1.0' -NotAfter (Get-Date).AddYears(20) | Export-PfxCertificate -FilePath $NewPfxFilePath -Password $securePassword | Out-Null
    }
