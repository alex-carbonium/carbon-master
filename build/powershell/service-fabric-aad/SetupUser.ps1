...
$uri = [string]::Format($resourceUri, "tenantDetails", "")
...
{
    $uri = [string]::Format($resourceUri, "servicePrincipals", [string]::Format('&$filter=appId eq ''{0}''', $WebApplicationId))
...
$uri = [string]::Format($resourceUri, "applications", [string]::Format('&$filter=appId eq ''{0}''', $WebApplicationId))
...
$roleId = @{}
$userId = ""
$uri = [string]::Format($resourceUri, "users", "")
$newUser = @{
        accountEnabled = "true"
...
#User Role
$uri = [string]::Format($resourceUri, [string]::Format("users/{0}/appRoleAssignments", $userId), "")
$appRoleAssignments = @{
    id = $roleId
    principalId = $userId
    principalType = "User"
