...
{
    $uri = [string]::Format($resourceUri, "applications", [string]::Format('&$filter=displayName eq ''{0}''', $WebApplicationName))
...
{
    $uri = [string]::Format($resourceUri, "applications", [string]::Format('&$filter=displayName eq ''{0}''', $NativeClientApplicationName))
    $apps += (Invoke-RestMethod $uri -Headers $headers).value
...
foreach($app in $apps)
{
    ...
    {
        #service principal
        $uri = [string]::Format($resourceUri, "servicePrincipals", [string]::Format('&$filter=appId eq ''{0}''', $app.appId))
        $servicePrincipalId = (Invoke-RestMethod $uri -Headers $headers).value.objectId

        if($servicePrincipalId -ne $null){
            $uri = [string]::Format($resourceUri, [string]::Format("servicePrincipals/{0}/appRoleAssignedTo", $servicePrincipalId), "")
            foreach($appRole in (Invoke-RestMethod $uri -Headers $headers).value){
                ...
                Invoke-Expression ".\CleanupUser.ps1 $args" 
            }
        }
    }

    Write-Host 'Deleting Application objectId = '$app.objectId
    $uri = [string]::Format($resourceUri, [string]::Format("applications/{0}",$app.objectId), "")
