...
$resourceUri = "https://graph.windows.net/" + $TenantId + "/{0}?api-version=1.5"
$ConfigObj = @{}
...
$uri = [string]::Format($resourceUri, "applications")
$webApp = @{
...
#Service Principal
$uri = [string]::Format($resourceUri, "servicePrincipals")
$servicePrincipal = @{
    accountEnabled = "true"
    appId = $webApp.appId
...
$uri = [string]::Format($resourceUri, "servicePrincipals") + '&$filter=appId eq ''00000002-0000-0000-c000-000000000000'''
$AADServicePrincipalId = (Invoke-RestMethod $uri -Headers $headers).value.objectId

$uri = [string]::Format($resourceUri, "oauth2PermissionGrants")
$oauth2PermissionGrants = @{
    ...
    expiryTime = (Get-Date).AddDays(180).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ss.fffffff")
}
